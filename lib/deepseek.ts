...
const VALID_COLORS: EventColor[] = ["blue", "green", "red", "yellow", "purple", "pink", "orange", "teal"];
 ...
 */
export async function generateSchedule(options: GenerateScheduleOptions): Promise<GenerateScheduleResponse> {
...
function validateSettings(raw: any): InferredSettings {
    ...
    let start = typeof raw.workingHoursStart === 'number' ? Math.max(0, Math.min(23, raw.workingHoursStart)) : 8;
    let end = typeof raw.workingHoursEnd === 'number' ? Math.max(1, Math.min(24, raw.workingHoursEnd)) : 18;

    // Ensure start < end
    if (end <= start) end = start + 1;

    return {
        weekStartsOnSunday: typeof raw.weekStartsOnSunday === 'boolean' ? raw.weekStartsOnSunday : true,
        use12HourFormat: typeof raw.use12HourFormat === 'boolean' ? raw.use12HourFormat : true,
        showDates: typeof raw.showDates === 'boolean' ? raw.showDates : true,
        workingHoursStart: start,
        workingHoursEnd: end,
        timeIncrement: validIncrements.includes(raw.timeIncrement) ? raw.timeIncrement : 60,
    };
}
    ...
    Object.keys(byDay).forEach(dayKey => {
        ...
        for (const current of dayEvents) {
            ...
            if (currStart < prevEnd) {
                // Overlap detected!
                console.warn(`Overlap detected: [${previous.title}] ends ${previous.endHour}:${previous.endMinute} vs [${current.title}] starts ${current.startHour}:${current.startMinute}`);
...
function validateEvent(
    ...
    if (eHour === 24) { eHour = 24; eMin = 0; } // Normalize midnight end
    ...
    return {
        ...
        description: typeof e.description === "string" ? e.description.trim().substring(0, 500) : "",
        day: e.day as number,
        startHour: sHour,
        startMinute: sMin,
        endHour: eHour,
