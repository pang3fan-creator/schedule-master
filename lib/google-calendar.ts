 ...
 */
async function getGoogleCalendarTimezone(
  ...
  try {
    ...
    if (timezone) {
      console.log(
        "[Calendar] User's Google Calendar timezone (from settings):",
        timezone,
      );
      return timezone;
    }
    ...
    console.log(
      "[Calendar] Settings API failed, trying calendars API...",
  ...
  try {
    ...
    if (timezone) {
      console.log(
        "[Calendar] User's Google Calendar timezone (from primary calendar):",
        timezone,
      );
      return timezone;
    }
    ...
    console.error(
      "[Calendar] Failed to get timezone from primary calendar:",
      error instanceof Error ? error.message : String(error),
    );
  }
...
function convertToGoogleEvent(
  ...
  console.log(
    `[Calendar] Converting event: ${event.title}, date: ${dateStr}, time: ${startTime}-${endTime}, timezone: ${timeZone}`,
  );
...
export async function syncEventsToGoogleCalendar(
  ...
  if (!calendar) {
    return {
      ...
      errors: [
        { eventId: "all", error: "Failed to create Google Calendar client" },
      ],
  ...
  console.log(
    `[Calendar] Syncing ${events.length} events, weekStart: ${weekStart.toISOString()}, weekStartsOnSunday: ${weekStartsOnSunday}, eventTimezone: ${eventTimezone}`,
  );
  console.log(
    `[Calendar] Existing mappings: ${Object.keys(existingMappings).length} events`,
  );
  ...
  for (const event of events) {
    ...
    try {
      if (existingGoogleId) {
        // 已存在映射，更新事件
        console.log(
          `[Calendar] Updating existing event "${event.title}" (Google ID: ${existingGoogleId})`,
        );
        ...
        if (response.data.id) {
          ...
          console.log(
            `[Calendar] Created event with Google ID: ${response.data.id}`,
          );
        }
        results.synced++;
      }
      ...
      if (
        ...
        console.log(
          `[Calendar] Event not found in Google Calendar, creating new...`,
        );
        try {
          const response = await calendar.events.insert({
            calendarId: "primary",
