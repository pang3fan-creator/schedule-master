...
export function useEventDrag({
  ...
  const calculateCollisionBounds = useCallback(
    (event: Event) => {
      ...
      let maxOffset =
        ((maxHour * 60 - eventDurationMinutes - eventStartMinutes) / 60) *
        rowHeight;
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      let offsetY = Math.max(
        collisionBoundsRef.current.minOffset,
    ...
    (event: Event): { top: string; height: string } => {
      ...
      if (dragState.eventId === event.id && dragState.originalEvent) {
        ...
        const newStartOffset =
          tempEvent.startHour - minHour + tempEvent.startMinute / 60;
        const newEndOffset =
          tempEvent.endHour - minHour + tempEvent.endMinute / 60;
