...
export function useDragToCreate({
  ...
  const [creatingEvent, setCreatingEvent] = useState<CreatingEvent | null>(
    null,
  );
  ...
  const calculateEventTimes = useCallback(
    (clientY: number, isEnd: boolean = false) => {
      ...
      const deltaMinutes =
        Math.round(deltaY / pixelsPerMinute / timeIncrement) * timeIncrement;
      ...
      const { minStart, maxEnd } = findCollisionBoundary(
        originalStartTotal,
        currentTotal,
        start.dayIndex,
      );
  ...
  const handleGlobalMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (Math.abs(e.clientY - dragStartRef.current.originalClickY) < 10)
        return;
  ...
  const handleGlobalTouchMove = useCallback(
    (e: TouchEvent) => {
      ...
      // Prevent jitter/shrinking on small moves (threshold 10px)
      if (Math.abs(touch.clientY - dragStartRef.current.originalClickY) < 10)
        return;
  ...
  const handleGlobalTouchEnd = useCallback(
    (e: TouchEvent) => {
      ...
      const clientY =
        ...
        dragStartRef.current?.originalClickY ??
        0;
      handleEnd(clientY);
  ...
  const initDrag = useCallback(
    (clientY: number, cellTop: number, hour: number, dayIndex: number) => {
      const startH = hour;
      const startM = 0;

      dragStartRef.current = {
        y: cellTop,
        originalClickY: clientY,
        hour: startH,
        minute: startM,
        dayIndex,
      };
