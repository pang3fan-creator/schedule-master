...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
    ...
    const blogUrl = locale === 'en' ? `${baseUrl}/blog/${slug}` : `${baseUrl}/${locale}/blog/${slug}`;
...
function generateArticleSchema(post: ReturnType<typeof getPostBySlug>, slug: string, locale: string) {
    if (!post) return null;
...
function parseMarkdown(content: string) {
    ...
    const formatInline = (text: string) => {
        return text
            ...
            .replace(/\*(.*?)\*/g, "<em>$1</em>")
            .replace(/`(.*?)`/g, "<code class='bg-gray-100 dark:bg-gray-700 px-1 rounded text-gray-900 dark:text-gray-100'>$1</code>")
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith("> ")) {
            flushList();
            elements.push(
                <blockquote key={key++} className="border-l-4 border-blue-500 pl-4 py-2 my-6 italic text-gray-600 dark:text-gray-300 bg-blue-50/50 dark:bg-blue-900/20 rounded-r">
        ...
        if (line.trim().startsWith("|") && line.trim().endsWith("|")) {
            ...
            if (tableLines.length >= 2) {
                ...
                elements.push(
                    <div key={key++} className="my-6 overflow-x-auto rounded-lg border-2 border-gray-300 dark:border-gray-600 shadow-md">
                        <table className="min-w-full border-collapse">
                            <thead className="bg-gray-100 dark:bg-gray-800">
                                <tr>
                                    {headerCells.map((cell, cellIndex) => (
                                        <th key={cellIndex} className="border-b-2 border-r border-gray-300 dark:border-gray-600 last:border-r-0 px-4 py-3 text-left text-sm font-bold text-gray-800 dark:text-gray-200" dangerouslySetInnerHTML={{ __html: formatInline(cell) }} />
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {dataRows.map((row, rowIndex) => (
                                    <tr key={rowIndex} className={rowIndex % 2 === 0 ? "bg-white dark:bg-gray-900" : "bg-gray-50 dark:bg-gray-800"}>
                                        {row.map((cell, cellIndex) => (
                                            <td key={cellIndex} className="border-b border-r border-gray-200 dark:border-gray-700 last:border-r-0 px-4 py-3 text-sm text-gray-700 dark:text-gray-300" dangerouslySetInnerHTML={{ __html: formatInline(cell) }} />
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
        ...
        flushList();
        elements.push(
            <p key={key++} className="text-gray-700 dark:text-gray-300 leading-relaxed my-4" dangerouslySetInnerHTML={{ __html: formatInline(line) }} />
        );
    }
