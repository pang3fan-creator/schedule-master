...
function generateEventsFromTemplate(
...
function TemplatePreview({
  ...
  const eventsBySemanticDay = events.reduce(
    (acc, event, index) => {
    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-800/80 relative">
      ...
      <div className="grid grid-cols-8 border-b border-gray-200 dark:border-gray-700">
        ...
        {localizedDays.map((day) => (
          <div
            key={day}
            className="p-2 text-center text-xs font-medium text-gray-600 dark:text-gray-400 border-l border-gray-200 dark:border-gray-700"
          >
            {day}
          </div>
        ))}
      ...
      <div
        ...
        style={{ gridTemplateRows: `repeat(${hours.length}, 32px)` }}
      >
        {hours.map((hour, hourIndex) => (
          <React.Fragment key={hour}>
            <div
              key={`hour-${hour}`}
              className="p-1 text-xs text-gray-400 dark:text-gray-500 text-right pr-2 border-t border-gray-200 dark:border-gray-700"
            >
              {formatHour(hour)}
            </div>
            ...
            {localizedDays.map((_, dayIndex) => {
              ...
              return (
                ...
                >
                  {hourIndex === 0 &&
                    eventsBySemanticDay[semanticDay]?.map((event) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`text-[8px] ${colorConfig.text} p-0.5 block truncate`}
                          >
                            {event.title}
                          </span>
                        </div>
...
export function TemplateDetailClient({
  ...
  const getTemplatesUrl = () =>
    locale === "en" ? "/templates" : `/${locale}/templates`;
  const getHomeUrl = () => "/";
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="p-6 rounded-xl bg-gray-50 dark:bg-gray-800/80 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
              <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-2">
                {template.slug === "ai-schedule-builder"
                  ? t("detail.featureAIGeneration")
                  : t("detail.featurePreConfigured")}
              </h3>
              <p className="text-sm text-gray-600 dark:text-gray-300">
                {template.slug === "ai-schedule-builder"
                  ? t("detail.featureAIGenerationDesc")
                  : t("detail.featurePreConfiguredDesc", {
