...
const DeleteConfirmDialog = dynamic(
  () =>
    import("@/components/DeleteConfirmDialog").then(
      (m) => m.DeleteConfirmDialog,
    ),
  { ssr: false },
);
const ConflictDialog = dynamic(
  () => import("@/components/ConflictDialog").then((m) => m.ConflictDialog),
  { ssr: false },
);
const EditEventDialog = dynamic(
  () => import("@/components/EditEventDialog").then((m) => m.EditEventDialog),
  { ssr: false },
);
const AddEventDialog = dynamic(
  () => import("@/components/AddEventDialog").then((m) => m.AddEventDialog),
  { ssr: false },
);

// Dynamically import welcome tip components (shown once per user)
const MobileWelcomeTip = dynamic(
  () => import("@/components/MobileWelcomeTip").then((m) => m.MobileWelcomeTip),
  { ssr: false },
);
const DesktopWelcomeTip = dynamic(
  () =>
    import("@/components/DesktopWelcomeTip").then((m) => m.DesktopWelcomeTip),
  { ssr: false },
);
...
export default function ScheduleBuilderPage() {
  ...
  // Build FAQ array from translations for SEO schema
  const faqs = [
  ...
  const [mobileActionEvent, setMobileActionEvent] = useState<Event | null>(
    null,
  );
  ...
  const handleOpenAddDialog = useCallback(
    (data: { startTime: string; endTime: string; day: number }) => {
  ...
  useEffect(() => {
    ...
    const shouldOpenAIAutofill = localStorage.getItem(
      SHOULD_OPEN_AI_AUTOFILL_KEY,
    );
  ...
  const handleEventUpdate = useCallback(
    (updatedEvent: Event, skipConflictCheck?: boolean) => {
  ...
  const handleConflictDeleteExisting = useCallback(() => {
    ...
    setEvents((prevEvents) =>
      prevEvents.filter((e) => !conflictIds.includes(e.id)),
    );
  ...
  const handleConfirmDelete = useCallback(() => {
    if (deleteConfirm) {
      setEvents((prevEvents) =>
        prevEvents.filter((e) => e.id !== deleteConfirm.event.id),
      );
  ...
  return (
    <>
      <div
        className={`flex flex-col bg-gray-50 dark:bg-gray-950 ${isExporting ? "" : "h-screen overflow-hidden"}`}
      >
        ...
        {!isExporting && <h1 className="sr-only">{t("seo.h1")}</h1>}
        ...
        <div className={`flex flex-1 min-h-0 ${isExporting ? "" : "pt-16"}`}>
          ...
          <main
            className={`flex-1 min-h-0 overflow-auto ${!isExporting && isMobile ? "pb-20" : ""}`}
            ref={calendarRef}
          >
            {viewMode === "week" ? (
              <WeeklyCalendar
