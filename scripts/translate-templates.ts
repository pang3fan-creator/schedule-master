...
async function translateText(
  ...
  if (text.length <= MAX_CHARS) {
    ...
    try {
      ...
      console.error(
        `Error translating text: "${text.substring(0, 20)}..."`,
        err,
      );
...
async function translateTemplates() {
  ...
  console.log(
    `Found ${templateSlugs.length} templates to translate to Spanish (ES)...\n`,
  );
  ...
  for (const slug of templateSlugs) {
    ...
    if (template.longDescription) {
      translated.longDescription = await translateText(
        client,
