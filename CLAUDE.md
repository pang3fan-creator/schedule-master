...
// ✅ 正确：独立状态
const [isCalendarOpen, setIsCalendarOpen] = useState(false)        // 桌面端
const [isMobileCalendarOpen, setIsMobileCalendarOpen] = useState(false) // 移动端
```

#### 弹窗关闭行为
- **桌面端**：点击日期/Go to Today 后自动关闭
- **移动端**：同样需要自动关闭（使用 `isMobileCalendarOpen` 状态）
...
### 问题 1：移动端日历弹窗不关闭
**原因**：共享状态导致冲突
**解决**：使用独立的 `isMobileCalendarOpen` 状态

### 问题 2：拖拽功能在移动端不工作
**原因**：未处理触摸事件
**解决**：同时实现 `onMouseDown` 和 `onTouchStart` 处理器

### 问题 3：时间显示格式不正确
**原因**：未考虑用户设置（12/24 小时制）
**解决**：使用 `formatTime()` 工具函数，读取用户设置

### 问题 4：事件位置计算错误
**原因**：行高度（rowHeight）计算不准确
**解决**：动态计算行高度，考虑移动端和桌面端差异
...
### 项目文档
- [命令行交互教程](./0-process/Claude_Code命令行交互教程.md)
- [危险模式说明](./0-process/Claude_Code危险模式说明.md)
- [AI 编程最佳实践](./0-process/AI编程最佳实践.md)
- [项目心得](./0-process/项目心得.md)
