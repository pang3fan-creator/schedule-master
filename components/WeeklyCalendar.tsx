...
export function WeeklyCalendar({
  ...
  const [rowHeight, setRowHeight] = useState(
    exportMode ? EXPORT_ROW_HEIGHT : 58,
  );
  ...
  const currentWeekStart = useMemo(
    () => getWeekStart(selectedDate, weekStartsOnSunday),
  ...
  const {
    creatingEvent,
    handleGridMouseDown: handleCreateMouseDown,
    handleGridTouchStart: handleCreateTouchStart,
  } = useDragToCreate({
    onAddEvent,
    rowHeight,
    timeIncrement,
  ...
  return (
    <div
      className={`flex flex-col p-2 md:p-6 bg-gray-50 dark:bg-gray-900/20 ${exportMode ? "" : "h-full"}`}
    >
      ...
      <div className="mb-1 md:mb-4 flex-shrink-0">
        ...
        <div className="hidden md:flex items-center justify-between relative">
          ...
          {showDates && (
            <div
              className={`flex items-center ${exportMode ? "flex-1 justify-center" : "absolute left-1/2 -translate-x-1/2"}`}
            >
              {!exportMode && (
                <Button
                  variant="ghost"
                  size="icon"
                  className="size-10 text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-100 hover:bg-gray-200 dark:hover:bg-gray-700"
              ...
              <h2
                className="text-xl font-semibold text-gray-900 dark:text-gray-100 w-[450px] text-center flex justify-center"
          ...
          <div className="md:hidden flex flex-col items-center">
            ...
            {!exportMode && (
              <div className="flex items-center justify-center w-full">
                ...
                <h2
                  className="text-sm font-semibold text-gray-900 dark:text-gray-100 text-center flex-1 px-1 flex justify-center"
                  suppressHydrationWarning
                >
                  <Popover
      ...
      <div
        className={
          exportMode
            ? ""
            : "flex-1 min-h-0 overflow-y-auto overflow-x-hidden md:overflow-auto"
        }
      >
        <div
          ref={gridRef}
        ...
        >
          ...
          {days.map((day) => (
            <div
              key={day.short}
              className="flex flex-col items-center justify-center border-b border-gray-300 dark:border-gray-700"
            >
          ...
          {hours.map((hour, index) => (
            <React.Fragment key={hour}>
              ...
              <div className="flex items-start justify-end pr-1 md:pr-3">
                <span className="text-[9px] md:text-xs font-medium text-gray-500 dark:text-gray-400 -translate-y-1/2 whitespace-nowrap">
                  {formatHour(hour, use12HourFormat, locale)}
                </span>
              </div>
              ...
              {index !== hours.length - 1 &&
                days.map((day, dayIndex) => (
                  ...
                  >
                    ...
                    {index === 0 &&
                      creatingEvent &&
                      creatingEvent.dayIndex === dayIndex && (
                        <div
                    ...
                    {index === 0 &&
                      (
                        eventsByDay.get(
                          getActualDay(dayIndex, weekStartsOnSunday),
                        ) || []
                      ).map((event) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              transition: isDragging
                                ? "none"
                                : "top 0.1s ease-out, height 0.1s ease-out",
                              userSelect: "none",
                            }}
                          ...
                          >
                            ...
                            {priorityModeEnabled && event.priority && (
                              <div
                                className={`absolute top-1 left-1 size-2 md:size-2.5 rounded-full ${PRIORITY_COLORS[event.priority].dot}`}
                                title={`${PRIORITY_COLORS[event.priority].label} Priority`}
                              />
                            )}
                            ...
                            <div className="flex items-center justify-center gap-1 overflow-hidden px-1">
                              {event.icon && eventIconMap[event.icon] && (
                                <span className="shrink-0 scale-75 md:scale-90">
                                  {React.createElement(
                                    eventIconMap[event.icon],
                                    {
                                      className: `size-3 md:size-3.5 ${colorConfig.text}`,
                                    },
                                  )}
                                </span>
                              )}
                              <p
                                ...
                                } ${event.task?.isCompleted ? "line-through opacity-60" : ""} truncate`}
                              >
                                {event.title}
                              </p>
                            </div>
                            ...
                            {shouldShowTime(displayMode) && (
                              ...
                              >
                                {formatTime(
                                  displayTime.startHour,
                                  displayTime.startMinute,
                                  use12HourFormat,
                                  locale,
                                )}{" "}
                                -{" "}
