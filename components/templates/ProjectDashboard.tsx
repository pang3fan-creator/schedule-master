...
export function ProjectDashboard({
  ...
  const fetchWeather = (lat: number, long: number) => {
    fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${long}&daily=weather_code&timezone=auto&forecast_days=3`,
    )
      .then((res) => res.json())
      .then((data) => {
        ...
        if (badWeatherDayIndex === 0) {
          ...
          setWeatherCheck({
            type: "alert",
            key: "forecastAlert",
            params: { days: badWeatherDayIndex },
          });
        } else {
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            title={t("title")}
          >
            {isOpen ? <X className="size-6" /> : <HardHat className="size-6" />}
          </Button>
        </PopoverTrigger>
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent
        align="start"
        side="right"
        className="w-[320px] p-0 overflow-hidden ml-2 shadow-xl border-orange-100 dark:border-gray-800"
      >
        <DashboardContent
          progress={progress}
          milestones={milestones}
    ...
    <div className="bg-white dark:bg-gray-900">
      ...
      <div className="p-4 bg-gray-50/50 dark:bg-gray-900/50">
        <h4 className="flex items-center justify-between text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3">
          ...
          <span className="text-[9px] font-normal text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800/50 px-1.5 py-0.5 rounded">
            {t("clickToUpdate")}
          </span>
        </h4>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-gray-500 dark:text-gray-400">
                  {milestone.week
                    ? t("week", { num: milestone.week.num })
                    : milestone.weekRange
                      ? t("weekRange", { range: milestone.weekRange.range })
                      : ""}
                </div>
              </div>
              {milestone.status === "in-progress" && (
            ...
            {weather?.type === "alert" ? (
              <>
                ...
                <p className="text-xs text-amber-800/80 dark:text-amber-400/90 leading-snug">
                  ...
                  {weather ? t(weather.key as any, weather.params) : ""}
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-xs text-emerald-800/80 dark:text-emerald-400/90 leading-snug">
                  ...
                  {weather
                    ? t(weather.key as any, weather.params)
                    : t("checkingForecast")}
                </p>
              </>
            )}
