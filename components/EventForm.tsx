...
import { Tag, Calendar, Clock, Info, Palette } from "lucide-react";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
export function EventForm({
  ...
  const iconOptions = [{ name: "None", icon: Tag }, ...ICON_OPTIONS];

  const SelectedIconComponent =
    iconOptions.find((i) => i.name === selectedIcon)?.icon || Tag;
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-2 flex-1">
            ...
            <Popover>
              ...
              <PopoverContent
                className="w-64 p-2 grid grid-cols-6 gap-1 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700"
                align="end"
              >
                {iconOptions.map((opt) => (
                  <Button
                    ...
                    onClick={() =>
                      onIconChange(opt.name === "None" ? undefined : opt.name)
                    }
                    title={t(
                      `eventDialog.form.icons.${opt.name.toLowerCase() === "none" ? "none" : opt.name}`,
                    )}
                  >
      ...
      <div className="space-y-2">
        <Label className="text-sm font-medium text-gray-700 dark:text-gray-200">
          {selectionMode === "single"
            ? t("eventDialog.form.day")
            : t("eventDialog.form.days")}
        </Label>
          ...
          <div className="flex flex-wrap gap-1.5 flex-1">
            {dayOptions.map((day) => {
              ...
              const translatedLabel = t(
                `eventDialog.form.dayLabels.${dayLabelKeys[day.value]}`,
              );
              return (
                <Button
                  key={day.value}
                  variant="outline"
                  size="sm"
                  className={`h-9 px-2 md:px-3 text-xs font-medium transition-all ${
